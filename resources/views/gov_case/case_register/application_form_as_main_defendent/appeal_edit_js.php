...
$(document).ready(function() {
    ...
    jQuery('select[name="case_category"]').on('change', function() {
        ...
        if (dataID) {
            jQuery.ajax({
                url: '{{ url(' /
                    ') }}/cabinet/case/dropdownlist/getdependentcasecategorytype/' +
                    dataID,
                type: "GET",
                dataType: "json",
    ...
    jQuery('select[name="case_category_origin"]').on('change', function() {
        ...
        if (dataID) {
            jQuery.ajax({
                url: '{{ url(' /
                    ') }}/cabinet/case/dropdownlist/getdependentorigincasenumber/' +
                    dataID,
                type: "GET",
                dataType: "json",
                success: function(data) {
...
function addBibadiRowFunc() {
    ...
    function Item(count, type = NULL) {
        ...
        items += '<tr id="bibadi_' + (count) + '">';
        items +=
            '<td><select name="other_respondent[]" class="form-control form-control-sm other_respondentCls" required><option value="">-- নির্বাচন করুন --</option>@foreach ($ministrys as $value)<option value="{{ $value->doptor_office_id }}" {{ old('
        ministry ') == $value->doptor_office_id ? '
        selected ' : '
        ' }}> {{ $value->office_name_bn }} </option>@endforeach</select><span class="text-danger d-none vallidation-message">This field can not be empty</span></td>';
        items += '<input type="hidden" name="bibadi_id[]" value="">';

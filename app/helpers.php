...
if (!function_exists('convertToBanglaDate')) {
    ...
    {
        // Define Bangla numbers
        $banglaNumbers = ['০', '১', '২', '৩', '৪', '৫', '৬', '৭', '৮', '৯'];

        // Define English numbers
        $englishNumbers = range(0, 9);

        // Split the date string into individual characters
        $chars = str_split($dateString);

        // Initialize a variable to store the Bangla date
        $banglaDate = '';

        // Iterate over each character in the date string
        foreach ($chars as $char) {
            // If the character is an English digit, replace it with the corresponding Bangla digit
            if (in_array($char, $englishNumbers)) {
                $banglaDate .= $banglaNumbers[$char];
            } else {
